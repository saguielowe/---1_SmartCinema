...
这份是最终工作边界和接入状态。A/B/C/D 的功能、评分修正和管理员场次运营台均已进入 `dev`。

| 模块 | 主要工作 | 主要文件 | 交付给谁 |
| --- | --- | --- | --- |
| A：选座与运营台（已合入 `dev`） | Canvas 座位图、弧形排布、单选/Ctrl 多选、推荐高亮；管理员场次概览、只读座位图、场次订单、满意度与跨场比较 | `js/seat-map.js`、`js/admin-dashboard.js`、`js/app.js`、`index.html`、`css/style.css` | 选座结果与可演示的管理员运营视图 |
| B：推荐与评分（已合入 `dev`） | 连座搜索、年龄限制、推荐理由、系统体验评分、观众手动评分及综合结果 | `js/recommendation.js`、`js/app.js`、`css/style.css` | 推荐结果和评分展示给 A/D |
| C：账号与订单（已合入 `dev`） | 注册登录、游客会话、LocalStorage、订单、锁票、热度数据、观众评分持久化接口 | `js/store.js`、`js/mock-data.js` | 座位状态、订单、评分和热度数据给 A/B/D |
...
6. B 的观众评分修正已接入已支付订单；A 在集成审查中修复了推荐转手动拖选的回归，并把 1–5 分纯观众满意度接入管理员逐场与全部场次视图。

## 分支约定

- `main`：只放可以打开演示的版本。
- 已使用的功能分支包括 `feature/canvas-seatmap`、`feature/recommendation`、`feature/order-storage`、`feature/ui-accessibility`、`feature/admin-dashboard`、`feature/manual-viewing-rating`。
