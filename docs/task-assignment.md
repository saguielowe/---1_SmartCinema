...
这份是当前的工作边界和接入状态。A/C 已进入 `dev`；B、D 开始合并前应先同步最新 `origin/dev`，避免在旧版 `app.js` 上继续扩展。

| 模块 | 主要工作 | 主要文件 | 交付给谁 |
| --- | --- | --- | --- |
| A：选座组件（已合入 `dev`） | Canvas 座位图、弧形排布、单选/Ctrl 多选、推荐高亮 | `js/seat-map.js` | `selectedSeatIds` 给 B/C/D |
| B：推荐与评分（待合并） | 连座搜索、年龄限制、推荐理由、体验评分 | `js/recommendation.js` 或独立模块 | `recommendationResult` 给 A 和 D |
| C：账号与订单（已合入 `dev`） | 注册登录、游客会话、LocalStorage、订单、锁票、热度数据 | `js/store.js`、`js/mock-data.js` | 座位状态、订单和热度数据给 A/B/D |
| D：页面整合（待完成） | 正式登录页、订单中心扩展、响应式、无障碍、联调和文档收口 | `index.html`、`css/style.css`、`js/app.js` | 可演示的完整流程 |
...
3. B 从 `store.getSeatStateBySchedule()` 等公共接口读取数据，输出标准座位 ID 和推荐理由，不直接修改 Canvas/订单状态。
4. D 在现有主流程上整合正式登录、订单分页/筛选、响应式和无障碍体验。
