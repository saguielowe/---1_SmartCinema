...
- `group` 会限制在 5 到 20 人；低于 5 按 5 人搜索，高于 20 按 20 人搜索，并在 `warnings` 中说明。
...
- 情侣票：优先中间区域连续双座。
- 家庭票：优先中后排连续座位。
- 团体票：5 到 20 人，同排连续，不拆分。
- 有少年和老人同时出现时，同时应用两条年龄限制，只在中间可用排里搜索。
- 需要无障碍时，候选连座中至少包含一个 `W` 座位。
...
| 指标 | 权重 | 实现方式 |
| --- | --- | --- |
| 视角 `angle` | 35% | 根据座位组中心点距离影厅中轴线的偏移计算，越靠中间越高 |
| 距离 `distance` | 35% | 根据平均排位与目标舒适排位的差值计算，家庭/后排偏好会把目标区后移 |
| 周边空位 `spacing` | 20% | 统计同排左右和前后相邻座位中仍可用的比例，空位越多干扰越少 |
| 偏好匹配 `preference` | 10% | 根据中间、后排、过道、票种特点和连座人数补充分 |

推荐理由直接来自这些评分项和规则命中情况，保证结果可解释，便于写入报告。

## 已知边界

- 当前模块只负责推荐和评分，不负责锁票、订单和手动点击选座。
- 若影厅某一侧连续座位数不足，团体票可能无法找到结果，需要换场次或减少人数。
